...
object TimeUsage {
  ...
  def dfSchema(columnNames: List[String]): StructType = columnNames match {
    case h::t => StructType(StructField(h, StringType, false) :: t.map(StructField(_, DoubleType, false)))
  }
  ...
  def classifiedColumns(columnNames: List[String]): (List[Column], List[Column], List[Column]) = {
    ...
    def contains(xs: List[String], s: String) = xs.exists(s.contains(_))

    def toColumns(names: List[String]) = names.map(new Column(_))

    def seqOp(acc: (List[String], List[String], List[String]), next: String) =
      if (contains(primary, next))
        (next +: acc._1, acc._2, acc._3)
      else if (contains(working, next))
        (acc._1, next +: acc._2, acc._3)
      else if (contains(other, next))
        (acc._1, acc._2, next +: acc._3)
      else
        acc

    val (primaries, workings, others) = columnNames.foldLeft((List[String](), List[String](), List[String]()))(seqOp)

    (toColumns(primaries), toColumns(workings), toColumns(others))
  ...
  def timeUsageGroupedTyped(summed: Dataset[TimeUsageRow]): Dataset[TimeUsageRow] = {
    ...
    def round1(d: Double) = d * 10.0 / 10.0

    val aggregated = summed.groupByKey(row => (row.working, row.sex, row.age))
